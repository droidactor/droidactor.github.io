...
- [x] **색인 생성 요청 2건 추가 + 일일 할당량 실측 (2026-08-10 오후).**
      ...
      **그래서 이 계정·이 사이트의 하루 할당량은 12건이다**(오전 10 + 오후 2 통과, 13번째 거부).
      ...
      반영되지 않은 것은 정상이다.
- [ ] **남은 11개 URL 색인 요청.** 하루 할당량 때문에 다음 날로 넘어간다.
      블로그 글 2종 4개(`find-ls-plc-ip-address` · `identify-industrial-devices-by-port` 의 en/ko 짝)를
      먼저 넣어 블로그 8종을 끝내고, 그 다음 한국어 제품 7개(`/ko/bt-keyboard/` `/ko/bt-ppt/`
      `/ko/bt-mouse/` `/ko/wifi-scout/` `/ko/ssh-scout/` `/ko/yt-downloader/` `/ko/lgtv/`).
      아직 크롤이 닿지 않은 `/bt-ppt/` `/yt-downloader/` 는 8-08 에 이미 요청했으므로 다시 넣지 않는다
      (대화상자가 명시한다 — "페이지를 여러 번 제출해도 대기열 위치나 우선순위가 변경되지 않습니다").
      (이 숫자의 이력 — 처음에는 "남은 11개" 였고 그것은 sitemap 이 22 URL 이던 시점 기준이다.
      8-08 에 블로그 6종이 추가돼 34 URL 이 되면서 "남은 13개" 가 됐고, 8-10 오후 2건을 처리해 다시
      11개가 됐다. **지금의 11 은 맨 처음의 11 과 기준이 다르다 — 우연히 같은 숫자다.**
      34개 중 Google 이 아는 것은 9개다 — 2026-08-10 실측.)
