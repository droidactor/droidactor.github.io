...
/apps/idphoto/          제품 페이지 — Dotori ID Photo Maker          + /ko/apps/idphoto/
/manual/                사용 설명서 목록 — 6종 앱                     + /ko/manual/
/manual/<앱>/           6종 앱 영문 사용 설명서                       + /ko/manual/<앱>/
                        (bt-keyboard · bt-ppt · bt-mouse · wifi-scout · ssh-scout · lgtv)
/tech-notes/            개발 기술 노트 목록 — 9종 앱                 + /ko/tech-notes/
/tech-notes/<앱>/       9종 앱 영문 기술 노트                        + /ko/tech-notes/<앱>/
                        (bt-keyboard · bt-ppt · bt-mouse · wifi-scout · ssh-scout · lgtv · receipt · namecard · idphoto)
...
| `apps/index.html` · `ko/apps/index.html` | 14종 제품 목록 허브. 제품 상세와 7종 매뉴얼로 연결하고, 출시된 6종은 카드에 스크린샷 2장을 싣는다(각 장이 그 앱의 Play 스토어 링크) |
...
| `tech-notes/index.html` · `ko/tech-notes/index.html` | 기존 6종(`bt-keyboard`·`bt-ppt`·`bt-mouse`·`wifi-scout`·`ssh-scout`·`lgtv`)과 `receipt`·`namecard`·`idphoto` 의 9종 개발 기술 노트 목록 허브 |
| `tech-notes/<앱>/index.html` · `ko/tech-notes/<앱>/index.html` | 9종 앱의 한·영 기술 노트. 소스 기준 버전과 구조·protocol·핵심 설계 결정·안전 경계·확인된 제약을 기록하고 제품·매뉴얼·현장 노트와 연결한다 |
...
| `sitemap.xml` | 홈 2 + 앱 허브 2 + 제품 28 + 매뉴얼 허브 2 + 매뉴얼 14 + 기술 노트 허브 2 + 기술 노트 18 + 기술 노트 상세 8 + 블로그 허브 2 + 블로그 61 = 139개 URL(2026-09-23 기준). 모든 한·영 쌍에 `xhtml:link` hreflang 3개를 둔다 |
...
| 앱별 처리방침 | `#privacy-<app key>` — `keyboard` · `ppt` · `mouse` · `numpad` · `wifi` · `ssh` · `ytdl` · `lgtv` · `roman` · `currency` · `gas` · `apartment` · `calendar` · `unitconverter` · `unit` · `receipt` · `namecard` · `idphoto` (`apps.tsv` 등재 앱은 그 key 와 같다. 등재되지 않은 `gas`·`apartment`·`calendar`·`unitconverter` 는 이 표가 정본. 구 앱 `com.app.unitconverter` 는 `#privacy-unitconverter`, 새 앱 `com.droidactor.unit` 은 `#privacy-unit`) |
...
- 홈 2개, 앱 허브 2개, 제품 28개, 매뉴얼 허브 2개와 6종 한·영 매뉴얼 12개, 기술 노트 허브 2개와
  9종 한·영 기술 노트 18개, 주제별 한·영 상세 조사 글 8개. 매뉴얼과 기술 노트는 `TechArticle`, 제품은 `SoftwareApplication`으로
...
- **9종 앱 기술 노트를 고치면** 사용법이 아니라 구현 근거를 유지한다.
