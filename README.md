...
| 앱별 처리방침 | `#privacy-<app key>` — `keyboard` · `ppt` · `mouse` · `wifi` · `ssh` · `ytdl` · `lgtv` · `roman` · `gas` · `apartment` · `calendar` · `unit` (`apps.tsv` 등재 앱은 그 key 와 같다. 등재되지 않은 `gas`·`apartment`·`calendar`·`unit` 은 이 표가 정본) |
