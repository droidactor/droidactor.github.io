...
- [x] **Play Console 처리방침 URL 등록 확인 — 7/7 정확.** 2026-08-19 Console 실측이다.
      ...
      **⚠ 이 `7/7` 은 앵커가 8개였던 8-19 오전 시점의 값이다.** 같은 날 17:38 의 커밋 `5beaf34` 가
      ...
      11개), **그 3개에 대응하는 앱의 Console 등록은 아직 대조하지 않았다.** 2026-08-24 에 발견했다
      (§6 2026-08-24 성과 7번, 같은 절 다음 작업 10번). 분모가 바뀌었으므로 이 `[x]` 를 "전부 확인됨"
      으로 읽지 않는다.
...
- [ ] **`/apps/lgtv/`·`/ko/apps/lgtv/` 를 URL 검사로 다시 읽어 `Google에서 선택한 표준 URL` 이
      self 로 바뀌었는지 본다**(URL 검사는 할당량을 소모하지 않는다). 바뀌었으면 8-14 요청이 뒤늦게
      처리된 것이고, 그대로면 §13.4 의 마지막 줄대로 계속 기다린다.
- [ ] 같은 검사에서 **`최근 크롤링` 날짜가 2026-08-14 에서 움직였는지** 함께 본다. 이 값이 canonical
      재평가의 선행 조건이다.
- [ ] 다른 제품 표본(`/apps/bt-keyboard/`, `/apps/wifi-scout/`)도 같은 값을 읽어 §13.3 의 확산
      가설을 검증한다. 이 둘은 8-14 에 색인 요청한 URL 이므로 lgtv 와 조건이 같다.
