...
2. **2026-08-20 — sitemap 재제출 판정.** 그날에도 `가져올 수 없음`이면 기존 행을 삭제하지 않은 채
   같은 `sitemap.xml`을 1회 제출한다 (§4.2). **선결 조건 "live sitemap = 저장소 파일" 은 8-19
   08:32 KST 실측으로 해소됐다**(커밋 `54af9c8`, §12). 확인 전에 재제출했다면 첫 성공 수집이
   `2026-08-18` 이 박힌 낡은 sitemap 으로 이뤄졌을 것이다 — §10.1 이 8-17 에, §11.1 이 8-18 에
   각각 경계한 실패다. **8-20 에 또 배포를 한다면 재제출 전에 이 확인을 다시 한다.**
