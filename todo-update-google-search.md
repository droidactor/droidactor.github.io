...
- [x] 중요한 내용·구조·link가 바뀐 모든 URL의 `lastmod`가 실제 최종 변경일인지 양방향으로 확인한다.
      ...
      **`lastmod` 를 고칠 때는 같은 URL 의 페이지 `dateModified`·화면 표시 날짜를 짝으로 함께 본다.**
      현재 분포는
      `2026-08-14` 32 + `2026-08-15` 8 + `2026-08-16` 8 + `2026-08-18` 22 = 70 이다.
...
- [x] 기존 행의 상태·마지막으로 읽은 날짜·발견된 페이지 수를 기록한다.
      ...
      **2026-08-15 재확인 — 목록 행은 그대로이고, 세부 화면에서 위 해석이 정정됐다.**
      ...
      2026-08-14·15에는 재시도 흔적이 없다. 8-13은 URL 구조 개편(`f300f26`) 배포 전후 시점이라
      그때의 실패가 현재 sitemap(2026-08-15 실측 `200` + `application/xml` + well-formed + `<loc>` 66개)
      의 결함을 뜻하지는 않는다.
...
1. **[완료] §11.1 정정을 2026-08-19 에 배포했다.** 8-20 재제출 전에 마쳐야 했던 항목이다 — 미배포로
   재제출하면 첫 성공 수집이 틀린 `lastmod` 로 이뤄진다(§10.1 이 8-17 에 남긴 것과 같은 이유).
   **남은 것은 live 반영 확인 하나다** — push 후 `sitemap.xml` 이 저장소 파일과 같아지는지, 매뉴얼
   4개의 화면 표기가 `15 August 2026` / `8월 15일 갱신` 으로 바뀌었는지 본다.
2. **GSC sitemap 세부 화면을 열어 마지막으로 읽은 날짜를 기록한다.** 8-17 에는 목록 화면만 봤다.
   8-20 재제출 판정의 선결 확인이다 (§4.2).
...
4. **2026-08-20 — sitemap 재제출 판정.** 그날에도 `가져올 수 없음`이면 기존 행을 삭제하지 않은 채
   같은 `sitemap.xml`을 1회 제출한다 (§4.2).
...
8. **`bt-mouse` — 2026-08-20 까지 게시되지 않으면 배지·상태 문구를 게시 전 표기로 되돌린다** (§11.2).
   게시됐으면 스토어 링크 22곳이 `200` 인지 재확인한다. 404 링크를 품은 8개 페이지에 홈 2개가 들어
   있고, 그 8개가 지금 사이트에서 가장 신선한 `lastmod` 를 달고 있다.
9. **다음 배포 때 §11.1 재현 절차를 그대로 한 번 돌린다.** `git log -1` 을 기계적으로 쓰지 않는다 —
   이번 정정 커밋이 그 함정의 첫 사례다.
...
**정정은 2026-08-18 에 하고 배포는 2026-08-19 에 했다**(제목이 `날짜 신호 34건 정정…` 인 커밋).
...
이 8개가 이번에 `lastmod` `2026-08-18` 로 사이트에서 가장 신선한 신호를 받았고, 다음 작업은 색인 요청
